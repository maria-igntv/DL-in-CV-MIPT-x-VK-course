...
def export(checkpoint: str, out_path: str, lut_dim: int = 17, n_luts: int = 3, img_size: int = 512):
    ...
    # Свести внешние веса (*.onnx.data) в один model.onnx — удобнее для git и для Triton.
    try:
        import onnx

        m = onnx.load(out_path, load_external_data=True)
        onnx.save(m, out_path, save_as_external_data=False)
        print("  inlined external weights into single .onnx file")
    except Exception as exc:
        print(
            "  warning: could not inline external ONNX weights "
            f"(если нет model.onnx.data рядом с .onnx — это ожидаемо): {exc}"
        )

    print(f"Exported ONNX model to {out_path}")
    print("  opset=16  dynamic_axes=batch,height,width")
